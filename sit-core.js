...
const SIT = {
  ...
  go(cmd) {
    ...
    if (['CONFIG','CFG','SETTINGS'].includes(raw)) { window.location.href = 'config.html'; return; }
    // BACK
...
document.addEventListener('keydown', e => {
  if (e.key === 'F1') { e.preventDefault(); SIT.go('MON'); return; }
  if (e.key === 'F2') { e.preventDefault(); SIT.go('HELP'); return; }
  if (e.key === 'Escape') { document.getElementById('sit-upgrade-modal')?.remove(); return; }
  if (e.altKey) {
    if (!window.PANELS) return;
    const map = { n:'news', f:'fx', t:'teams', a:'alerts', w:'wei', k:'ticker' };
    const panel = map[e.key.toLowerCase()];
    if (panel) { e.preventDefault(); PANELS.toggle(panel); }
  }
});
